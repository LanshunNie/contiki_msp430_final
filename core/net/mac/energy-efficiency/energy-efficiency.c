...
#define BEHAVIOUR_LOCKED_THRESHOLD 3  
...
#define ACTIVE_BASE_CURRENT 700000
#define INACTIVE_BASE_CURRENT 4256
#define NOTHING_ACTIVE_BASE_CURRENT 270000

#define AUTO_CAL_BASECURRENT 1
#define DEDUG 0
/*---------------------------------------------------------------------------*/
...
static void adjust_energy_efficiency_effect(uint32_t present_cur);
...
#if AUTO_CAL_BASECURRENT
  static uint8_t cal_number_of_active(void);
  static uint8_t cal_number_of_1(uint8_t data);
  static void cal_energy_efficiency_base_conf(void);
  static uint8_t pre_active_count = 0;
...
energy_efficiency_cal_rdc_cycle_time(void)
{
  ...
  if(behaviour_tb_index >= NODE_BEHAVIOUR_TABLE_SIZE)
  {
    ...
    #if AUTO_CAL_BASECURRENT
      cal_energy_efficiency_base_conf();
    #endif

    old_cycle_time = energy_efficiency.rdc_cycle_time;
    
    // printf("present_cur %lu\n",present_cur );
      ...
      // if(present_cur  > (energy_efficiency.current_budget + energy_efficiency.guard_current)){
      if(present_cur  > energy_efficiency.current_budget ){
        if( energy_efficiency.energy_efficiency_effect == ENERGY_EFFICIENCY_INVALID ||                                                           \
...
/*---------------------------------------------------------------------------*/
static void  
adjust_energy_efficiency_effect(uint32_t present_cur)
{

  if( energy_efficiency.prev_current != 0 && energy_efficiency.cycle_time_direction == CYCLE_TIME_INCREASE){
    
    if(energy_efficiency.prev_current > present_cur){//energy efficiency effect
  ...
  if( energy_efficiency.prev_current != 0 && energy_efficiency.cycle_time_direction == CYCLE_TIME_DECREASE){
  ...
  if( energy_efficiency.prev_current != 0 && energy_efficiency.cycle_time_direction == CYCLE_TIME_MAINTAIN){
       //+ energy_efficiency.guard_current
    if(present_cur  < (energy_efficiency.current_budget) &&                                                                                      \
...
energy_efficiency_cal_present_current(void){
  ...
  uint64_t transmit =0,listen=0,lpm=0,cpu=0;
  ...
  adjust_energy_efficiency_effect(present_cur);

  energy_efficiency.prev_current = present_cur;
...
/*---------------------------------------------------------------------------*/
static uint8_t 
cal_number_of_active(void){
  uint8_t schedule_bit[18];
  uint8_t index=0;
  uint8_t active_count=0;
  schedule_bitmap_get(schedule_bit);
  for(;index<18;index++){
    active_count += cal_number_of_1(schedule_bit[index]);
  }
  return active_count;
}
/*---------------------------------------------------------------------------*/
static uint8_t 
cal_number_of_1(uint8_t data){
  uint8_t ret = 0;
  while (data != 0) {
...
/*---------------------------------------------------------------------------*/
static void 
cal_energy_efficiency_base_conf(void){

  uint8_t active_count=cal_number_of_active();
  uint8_t in_active_count= 0;
  uint16_t N_Active_count = 0;
  if(pre_active_count != active_count){

    #if LOW_LATENCY
      N_Active_count = (144 - active_count) * get_low_latency_active_second() / 600;
    #endif

    in_active_count = 144 - active_count -N_Active_count;
    
    energy_efficiency.current_budget = ((uint64_t)(active_count* ACTIVE_BASE_CURRENT) + (uint64_t)INACTIVE_BASE_CURRENT * in_active_count                            \
                                         + (uint64_t)(N_Active_count * NOTHING_ACTIVE_BASE_CURRENT))/ 144;// nA

    energy_efficiency.guard_current = 4000;// nA

    energy_efficiency.expect_current = energy_efficiency.current_budget;

    pre_active_count = active_count;
    behaviour_tb_index = NODE_BEHAVIOUR_TABLE_SIZE/2;
    // printf("active count %d\n",active_count );
    // printf("N_Active_count count %d\n",N_Active_count );
    // printf("active ac %lu \n", active_count* ACTIVE_BASE_CURRENT);
    //     printf("inactive ac %lu \n", INACTIVE_BASE_CURRENT * in_active_count);
    // printf("n active ac %lu \n", N_Active_count * NOTHING_ACTIVE_BASE_CURRENT);
    // printf("budegt %lu\n", energy_efficiency.current_budget);
  }

}

void reset_behaviour_tb_index(void){

  //zhangwei set changed for reset energy-efficiency information
  energy_efficiency.expect_current  = energy_efficiency.guard_current;

  energy_efficiency.rdc_cycle_time  = energy_efficiency.con_rdc_cycle_time ;

  energy_efficiency.energy_efficiency_effect = ENERGY_EFFICIENCY_INVALID;

  behaviour_tb_index = 0;
