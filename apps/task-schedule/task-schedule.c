...
static clock_time_t get_Start_Time(void){
    ...
    //   start_time = guard_time + start_index * TIME_SLOAT * 2 + calculate_hash()*time_interval() * 4 + (random_rand() % ( 2 * time_interval()) ) * get_rank();
    // }else{
    //   start_time = guard_time + (start_index+1) * TIME_SLOAT * 2 + time_interval() * 2 + (random_rand() % ( 2 * time_interval()) ) * get_rank();
    // }
